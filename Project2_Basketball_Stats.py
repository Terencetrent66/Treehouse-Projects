...
def menu_choice2():
    ...
    while submenu == 1:
        try:
            team_answer = int(input("Please enter 1 to see stats for Panthers, 2 to see stats for Bandits, 3 to see stats for Warriors:  "))
            if team_answer not in range(1, 4):
                print("Sorry, that number is not 1, 2, or 3.  Please try again.")
                continue
            if team_answer == 1:
                print("\nTeam: Panthers Stats\n---------------------")
